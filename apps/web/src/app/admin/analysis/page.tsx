...
function BudgetReservations({canReconcile}:{canReconcile:boolean}){
 ...
 return <section className="panel"><h2>Unresolved AI spending holds</h2>
  <p className="muted">Only holds older than {holds.data?.minimum_age_minutes??10} minutes appear. Compare each with the provider billing record before entering an actual cost. Unknown charges continue to count against the daily allowance.</p>
  ...
  {holds.data?.pending?.map((hold:any)=><article key={hold.id} className="panel">
   ...
   {canReconcile&&<form className="stack" onSubmit={event=>void reconcile(event,hold.id)}>
    <label className="field">Verified provider cost in cents<input name="actual_cents" type="number" min="0" step="0.000001" required/></label>
    <label className="field">Provider billing reference<input name="provider_reference" required minLength={8} maxLength={200}/></label>
    ...
    <label className="check-label"><input name="confirm" type="checkbox" required/> I checked the provider billing record and confirm this amount.</label>
