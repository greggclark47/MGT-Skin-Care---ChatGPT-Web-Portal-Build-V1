...
(async()=>{
 ...
 try{
  ...
  assert.equal(result.data.model,'gpt-6-astra');
  ...
  assert.equal((await request('/admin/ai/reservations')).data.pending.some(row=>row.id===reservation),true);
  assert.equal((await request('/admin/ai/reservations/reconcile',{id:reservation,actual_cents:0,provider_reference:'billing-check-001'})).status,400);
  assert.equal((await request('/admin/ai/reservations/reconcile',{id:reservation,actual_cents:0,provider_reference:'billing-check-001',confirm:true})).status,400);
  ...
  assert.equal((await request('/admin/ai/reservations/reconcile',{id:reservation,actual_cents:2,provider_reference:'billing-check-001',confirm:true})).status,403);
  await db.tx(async r=>{const account=await r.get('accounts','operator');await r.put('accounts','operator',{...account,roles:['superadmin']});});
  assert.equal((await request('/admin/ai/reservations/reconcile',{id:reservation,actual_cents:2,provider_reference:'billing-check-001',confirm:true})).status,200);
  assert.equal((await request('/admin/ai/reservations/reconcile',{id:reservation,actual_cents:0,provider_reference:'billing-check-001',confirm:true,confirmed_no_charge:true})).status,409);
  assert.equal((await budget.spentToday('other-account','tier1_copy')),2);
  console.log('Operator analysis auth, consent, scope, CSRF, model, budget and daily rate passed.');
